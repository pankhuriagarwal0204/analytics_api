...
class QuerysetProcessors:
    ...
    def get_verified_data_battalion(start, end, pk):
        verified = Intrusion.objects.filter(verified_datetime__gte=start, verified_datetime__lt=end, morcha__post__battalion=pk)
        return verified
...
class Widgets:
    ...
    """
    divided on basis of level, timespan
    """
    ...
    """
    takes old dangling intrusion, today's open intrusions
    """
    ...
    """
    currently it is on the count of
    intrusions verified as per detected
    """
    ...
    def get_vulnerable_post(self):
        if self.level == 'morcha':
            ...
            res = self.detected.values('morcha__post__name', 'morcha__post__uuid').annotate(count=Count('verified_datetime'))
        return res
